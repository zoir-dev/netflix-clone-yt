...
import { Add, Close, PlayArrow, ThumbUpOffAlt, VolumeOff, VolumeUp } from '@mui/icons-material'
import { Element, Genre } from '../pages/typing'
...
function Modal() {
    ...
    return <MuiModal open={showModal} onClose={() => setShowModal(false)}
        ...
        <>
            <Close onClick={() => setShowModal(false)}
                className='absolute right-5 top-5 p-1 !z-40 h-9 w-9 cursor-pointer
                 text-white bg-[#181818] border-none outline-none'
            ...
            <div className='relative pt-[56.25%]'>
                ...
                <div className='absolute bottom-10  flex  w-full items-center justify-between px-10'>
                    <div className='flex space-x-2'>
                        <button className='flex items-center gap-x-2 rounded bg-white  px-8 py-2 text-xl 
                        ...
                        <button className="modalButton">
                            <ThumbUpOffAlt className='h-7 w-7' />
                        </button>
                    </div>
                    <button onClick={() => setMuted(!muted)} className='modalButton'>
                        {muted ? <VolumeOff className='h-6 w-6' /> : <VolumeUp className='h-6 w-6' />}
                    </button>
                </div>
            </div>
            <div className='flex space-x-16 rounded-b-md  bg-[#181818]  px-10 py-10'>
                <div className='space-y-6 text-lg'>
                    ...
                    <div className='flex col gap-x-10 gap-y-4 font-light md:flex-row'>
